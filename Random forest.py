...
for seed in range(1, 101):
    ...
    for train_idx, test_idx in kfold.split(all_train_x, all_train_y):
        ...
        prob_pred = model.predict_proba(all_data_scale[:,:-1])
        prob_pred_first = [x[1] for x in prob_pred]
        all_prob.append(prob_pred_first)
...
out_result.append(all_data_scale[:,-1])
# out_result.columns = data_rock.iloc[:,:-1].columns
out_result.to_csv("C:/Users/DELL/OneDrive - cug.edu.cn/data_basalt/Results/all_prob.csv",encoding="utf_8_sig", header=False, index=False)